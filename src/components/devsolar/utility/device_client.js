...
export function detectDevice(userAgent, screenInfo = {}) {
  ...
  const checkUserAgent = (patterns) =>
    patterns.some((pattern) => pattern.test(ua));
  ...
  const consolePatterns = [/PlayStation/i, /Xbox/i, /Nintendo/i];
  ...
  if (
    checkUserAgent(desktopPatterns) ||
    (!device.type && !isMobileUA && !isTabletUA)
  ) {
    return 'Desktop';
  }
...
export default function DeviceDetector({ initialDeviceType }) {
  ...
  return (
    <div>
      <p>Dispositivo Detectado: {deviceType}</p>
    </div>
  );
}
